...
### Fix
- Stages 1 and 4 now stream via `claude_client.messages.stream()` instead of `messages.create()`. The SDK raises a `ValueError` client-side when a non-streaming request's estimated duration exceeds roughly ten minutes, and `MAX_OUTPUT_TOKENS=64000` at `xhigh` effort is well past that threshold — the request would have failed before reaching the API. At the previous 20000 the estimate stayed under the limit, which is why this surfaced only with the ceiling raise. `stream.get_final_message()` returns the accumulated `Message`, so `.content` and `.usage` are unchanged and no downstream code was affected. Streaming also avoids idle-connection drops on long generations. The pre-flight calls stay non-streaming; at 8000 and 4000 they are nowhere near the threshold.
